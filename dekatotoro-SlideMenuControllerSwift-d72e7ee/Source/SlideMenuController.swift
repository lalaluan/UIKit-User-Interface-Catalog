...
public class SlideMenuController: UIViewController, UIGestureRecognizerDelegate {
    ...
    public override func openLeft() {
        ...
        self.delegate?.leftWillOpen?()   //输出了语句leftWillOpen,但是函数具体在哪不知道
        
        setOpenWindowLevel()//设置windowLevel让新窗口在老窗口之上
        // for call viewWillAppear of leftViewController
        //不能直接调用childVC的viewWillAppear,用这个方法间接调用
        leftViewController?.beginAppearanceTransition(isLeftHidden(), animated: true)
        openLeftWithVelocity(0.0) //是控制左视图弹出的语句
    ...
    public func openLeftWithVelocity(velocity: CGFloat) {
        ...
        let finalXOrigin: CGFloat = 0.0  //控制弹出后，左视图最左边从哪里开始
        
        var frame = leftContainerView.frame;
        frame.origin.x = finalXOrigin;
        
        var duration: NSTimeInterval = Double(SlideMenuOptions.animationDuration) //左边栏弹出花费的时间
        if velocity != 0.0 { //滑动手势的时候，有个速度，用这个速度控制视图的滑出，如果是用案件触发的，就没有这个速度
        ...
        UIView.animateWithDuration(duration, delay: 0.0, options: UIViewAnimationOptions.CurveEaseInOut, animations: { [weak self]() -> Void in
            if let strongSelf = self {
                ...
                if let strongSelf = self {
                    strongSelf.disableContentInteraction()//取消对主界面的手势操作
    ...
    public func closeLeftWithVelocity(velocity: CGFloat) {
        ...
        UIView.animateWithDuration(duration, delay: 0.0, options: UIViewAnimationOptions.CurveEaseInOut, animations: { [weak self]() -> Void in
            if let strongSelf = self {
                ...
                if let strongSelf = self {
                    strongSelf.removeShadow(strongSelf.leftContainerView)//不知道shadow是指哪个
                    strongSelf.enableContentInteraction()//恢复对主界面的手势操作
                    strongSelf.leftViewController?.endAppearanceTransition()
    ...
    private func setUpViewController(targetView: UIView, targetViewController: UIViewController?) {
        if let viewController = targetViewController {
            ...
            targetView.addSubview(viewController.view)//让subview显示，这里相当于让left、main、rightview显示
